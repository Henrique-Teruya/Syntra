...
- **Estoque:** Operações de estoque utilizam a tabela `estoque_mov` (movimentações). O DAO possui método `getEstoque(int id_mov)` para busca direta por ID.

### Transações
- O `dao_demandas.inserirDadosComMateriais()` usa transação (`setAutoCommit(false)` + `commit`/`rollback`) para garantir atomicidade ao inserir demanda + materiais.
- Transações são usadas **apenas** quando há múltiplos inserts relacionados.

## 4. Interface Gráfica (Swing)
- **Padrão de Abas:** As telas utilizam consistentemente um `JTabbedPane` onde a aba de índice 0 é "Inserir" e a aba de índice 1 é "Visualizar".
...
- **Edição de Registros:** Ao editar um registro a partir de uma tabela, recupere o objeto completo do banco de dados via DAO usando o ID do registro (método `get[Entidade](id)`) para garantir que todos os campos subjacentes estejam devidamente preenchidos nos diálogos de edição.
...
- **Git:** Arquivos binários `.class`, o diretório `build/` e arquivos de IDE **não** devem ser commitados.
